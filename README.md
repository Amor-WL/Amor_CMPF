...
## 规范要求
1. **命名约定**：使用驼峰命名法，类名首字母大写，函数名首字母大写，变量名小写并使用下划线分隔。
2. **头文件**：使用#define防护符，避免使用#pragma once，按特定顺序包含头文件。
3. **作用域**：使用命名空间避免全局命名冲突，尽量减少全局变量的使用。
4. **类**：明确声明类的可拷贝性，使用访问控制符，合理组织成员声明顺序。
5. **函数**：编写简短、凝练的函数，合理使用函数重载和缺省参数。
6. **其他C++特性**：谨慎使用异常、RTTI等特性，优先使用C++风格的类型转换。
7. **注释**：为文件、类、函数和变量添加清晰的注释。
8. **格式**：使用2空格缩进，每行不超过80个字符，保持一致的代码格式。
